    ...
    action(action) {
        ...
        if (p.hero === 'aurel') {
            if (action === 'attack' && p.attackCooldown <= 0) {
                p.attack = .28;
                p.attackCooldown = .48;
                this.castBolt('light', 20, 12, 610, 1.05);
                return;
            }
            if (action === 'skill1' && p.skill1 <= 0) {
                p.skill1 = 9;
                p.attack = .3;
                p.shield = this.mode === 'pvp' ? 22 : 40;
                p.shieldTime = 5;
                this.emit('shield', { x: p.x, y: p.y - 55 });
                return;
            }
            if (action === 'skill2' && p.skill2 <= 0) {
                p.skill2 = 12;
                p.attack = .45;
                this.emit('healingWave', { x: p.x, y: p.y - 45 });
                if (!this.predicting)
        ...
        if (p.hero === 'sylva') {
            if (action === 'attack' && p.attackCooldown <= 0) {
                p.attack = .23;
                p.attackCooldown = .34;
                this.castBolt('arrow', 21, 12, 900, 1);
                return;
            }
            if (action === 'skill1' && p.skill1 <= 0) {
                p.skill1 = 5;
                p.attack = .35;
                this.castBolt('piercingArrow', 55, 25, 1050, 1.05, true);
                return;
            }
            if (action === 'skill2' && p.skill2 <= 0) {
                p.skill2 = 11;
                p.attack = .4;
                const center = Math.max(60, Math.min(4740, p.x + p.dir * 240));
                for (let i = -2; i <= 2; i++)
                    this.projectiles.push({ x: center + i * 62, y: p.y - 350 - Math.abs(i) * 35, vx: 0, vy: 650, life: .85, friendly: true, owner: p.id, kind: 'rainArrow', damage: 25, pvpDamage: 9, pierce: false, hits: new Set() });
                this.emit('arrowRain', { x: center, y: p.y - 45 });
                return;
            }
        }
        ...
        if (action === 'attack' && p.hero === 'kael' && p.attackCooldown <= 0) {
        ...
        if (action === 'skill1' && p.hero === 'kael' && p.skill1 <= 0) {
            p.skill1 = 5;
            p.attack = .4;
        ...
        if (action === 'skill2' && (p.hero === 'kael' || p.hero === 'lyra') && p.skill2 <= 0) {
            p.skill2 = 10;
            p.attack = .45;
