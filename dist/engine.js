...
export class World {
    ...
    constructor(chapter = 1) {
        ...
        if (this.chapter === 2)
            for (const e of this.enemies) {
                e.hp = e.maxHp = Math.round(e.maxHp * (e.kind === 'boss' ? 1.5 : 1.35));
                e.timer *= .8;
            }
    }
    ...
    damagePlayer(amount, sourceX) { const p = this.player; if (p.invincible > 0 || this.status !== 'playing')
        ...
        p.vx = 0;
        p.vy = 0;
        p.y = this.platforms.filter(b => p.x > b.x && p.x < b.x + b.w && b.y >= p.y).sort((a, b) => a.y - b.y)[0]?.y ?? 615;
        if (this.mode === 'pvp') {
    ...
    action(action) {
        ...
        if (action === 'attack' && p.hero === 'lyra' && p.attackCooldown <= 0) {
            this.comboStep = this.time - this.lastAttack < .9 ? (this.comboStep % 3) + 1 : 1;
            this.lastAttack = this.time;
            p.attack = .27;
            p.attackCooldown = this.comboStep === 3 ? .48 : .36;
            this.castFire(this.comboStep === 3 ? 26 : 18, this.comboStep === 3 ? 14 : 11);
            return;
        }
        if (action === 'skill1' && p.hero === 'lyra' && p.skill1 <= 0) {
            p.skill1 = 5;
            p.attack = .4;
            this.castFire(48, 24, true);
            return;
        }
        ...
        if (action === 'skill2' && p.skill2 <= 0) {
            ...
            this.emit(p.hero === 'lyra' ? 'inferno' : 'nova', { x: p.x, y: p.y - 45 });
            for (const e of this.enemies) {
                if (!e.dead && Math.hypot(e.x - p.x, e.y - p.y) < 285)
                    this.hitEnemy(e, p.hero === 'lyra' ? 64 : 76, e.x > p.x ? 1 : -1);
            }
            for (const target of this.players)
    ...
    stepPlayer(dt, input = {}) {
        ...
        if (p.y > 850) {
            p.invincible = 0;
            this.damagePlayer(this.chapter === 2 ? 24 : 16, p.x);
            p.x = this.checkpoint;
            p.y = p.hp > 0 ? 430 : 615;
            p.vy = 0;
            p.vx = 0;
    ...
    stepEnemies(dt) {
        for (const e of this.enemies) {
            ...
            if (!e.active)
                continue;
            e.dir = dx >= 0 ? 1 : -1;
            e.timer -= dt * (this.chapter === 2 ? 1.25 : 1);
            if (e.windup > 0) {
                e.windup -= dt;
                if (e.windup <= 0) {
                    if (e.kind === 'boss') {
                        if (e.phase % 2 === 0) {
                            ...
                            for (const member of this.players) {
                                if (member.hp > 0 && Math.abs(member.x - e.attackX) < 145 && member.y > 470) {
                                    this.player = member;
                                    this.damagePlayer(this.chapter === 2 ? 32 : 24, e.x);
                                }
                            }
                            this.player = p;
                            for (const dir of [-1, 1])
                                this.projectiles.push({ x: e.attackX, y: 594, vx: dir * (this.chapter === 2 ? 390 : 310), vy: 0, life: 2.1, friendly: false, hits: new Set() });
                        }
                        else {
                            for (let n = this.chapter === 2 ? -2 : -1; n <= (this.chapter === 2 ? 2 : 1); n++) {
                        ...
                        e.timer = e.hp < e.maxHp * .46 ? 1.0 : 1.65;
                        e.phase++;
                    }
                    else {
                        if (Math.abs(dx) < 105 && dy < 110)
                            this.damagePlayer((e.kind === 'bat' ? 9 : 13) * (this.chapter === 2 ? 1.3 : 1), e.x);
                ...
                if (e.kind === 'bat') {
                    ...
                    if (Math.abs(dx) > 60)
                        e.x += e.dir * (this.chapter === 2 ? 140 : 115) * dt;
                }
                else if (Math.abs(dx) > 64) {
                    const next = e.x + e.dir * (this.chapter === 2 ? 100 : 78) * dt;
            ...
            if (s.friendly) {
                ...
                if (this.mode === 'pvp')
                    for (const target of this.players) {
                        if (target.id !== s.owner && target.hp > 0 && !s.hits.has(target.id) && Math.abs(s.x - target.x) < 45 && Math.abs(s.y - (target.y - 50)) < 65) {
                            ...
                            this.hitOpponent(target, s.pvpDamage ?? 26);
                            if (s.kind === 'fireball') {
                                s.life = 0;
                                break;
                            }
                ...
                if (s.life > 0)
                    for (const e of this.enemies) {
                        if (!e.dead && !s.hits.has(e.id) && Math.abs(s.x - e.x) < (e.kind === 'boss' ? 100 : 55) && Math.abs(s.y - (e.y - (e.kind === 'boss' ? 100 : 40))) < 110) {
                            s.hits.add(e.id);
                            this.hitEnemy(e, s.damage ?? 58, Math.sign(s.vx));
                            if (s.kind === 'fireball') {
                                s.life = 0;
                                break;
                            }
                ...
                for (const p of this.players) {
                    if (p.hp > 0 && Math.abs(s.x - p.x) < 25 && Math.abs(s.y - (p.y - 45)) < 48) {
                        this.player = p;
                        this.damagePlayer(this.chapter === 2 ? 16 : 12, s.x);
                        s.life = 0;
                        break;
                    }
                }
    ...
    hazardState(offset) { const phase = (this.time + offset) % 5; return phase >= 4.2 ? 'active' : phase >= 3.2 ? 'warning' : 'idle'; }
