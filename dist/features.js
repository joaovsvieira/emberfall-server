...
export class Features {
    ...
    async loadMarket() { void this.hub.expansion?.transactions(); const epoch = ++this.marketEpoch, id = this.accountId; $('market-content').textContent = 'Carregando anúncios…'; await this.action(async () => { const q = $('market-query').value, slot = $('market-slot').value, mine = $('market-mine').checked; const data = await this.request(`market?page=${this.marketPage}&q=${encodeURIComponent(q)}&slot=${slot}${mine ? '&mine=1' : ''}`); if (epoch !== this.marketEpoch || id !== this.accountId)
