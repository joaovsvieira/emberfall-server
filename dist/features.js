...
export class Features {
    ...
    renderInventory() {
        ...
        const items = owned.inventory ?? [], backpack = items.filter((i) => !i.equipped && !i.listed), filtered = backpack.filter((i) => this.inventoryTab === 'other' ? (i.definition.hero && i.definition.hero !== hero) : (!i.definition.hero || i.definition.hero === hero));
        ...
        $('inventory-list').innerHTML = filtered.length ? filtered.map((i) => { const d = i.definition, gear = d.kind === 'equipment', recipe = d.kind === 'recipe'; return `<article class="item-card tier-${d.tier ?? 0}"><small>${slots[d.slot] ?? ''} · ${d.hero ? HEROES[d.hero].name : 'Todas as classes'}${i.quality ? ` · Mítica ${['', 'I', 'II', 'III'][i.quality]}` : ''}</small><strong>${esc(d.name)}</strong><p>${Object.entries(d.attributes).map(([k, v]) => `+${v} ${attr[k]}`).join(' · ') || esc(d.description ?? '')}</p><div class="item-actions">${gear ? `<button class="mini-primary" data-equip="${esc(i.id)}" ${d.hero !== hero || owned.level < d.requiredLevel ? 'disabled' : ''}>${d.hero !== hero ? 'Outra classe' : 'Equipar'}</button>` : recipe ? `<button class="mini-primary" data-learn="${esc(i.id)}" ${this.hub.profile.recipes?.includes(d.recipe) ? 'disabled' : ''}>${this.hub.profile.recipes?.includes(d.recipe) ? 'Já aprendida' : 'Aprender receita'}</button>` : d.kind === 'consumable' ? '<small>Selecione nos atalhos 1–5 acima.</small>' : ''}<button class="mini-button" data-sell="${esc(i.id)}">Vender</button></div></article>`; }).join('') : '<p class="empty-state">Nenhum item nesta categoria. Itens equipados e anúncios ativos ficam fora da mochila.</p>';
