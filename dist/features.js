...
const slots = { head: 'Cabeça', armor: 'Armadura', weapon: 'Arma', gloves: 'Luvas', boots: 'Botas', accessory: 'Acessório' };
...
export class Features {
    constructor(hub) {
        ...
        for (const id of ['ranking-mode', 'ranking-chapter'])
    ...
    renderInventory() {
        ...
        const items = owned.inventory ?? [];
        document.querySelectorAll('[data-slot]').forEach(b => { const slot = b.dataset.slot, item = items.find((i) => i.slot === slot && i.equipped); b.innerHTML = `${slots[slot]}<small>${item ? esc(item.definition.name) : 'Vazio'}</small>`; b.disabled = !item; b.title = item ? 'Clique para desequipar' : 'Equipe pelo inventário'; b.onclick = () => void this.action(async () => { await this.request('equip', { hero, item: item.id, equip: false }); await this.hub.refresh(false); }, b); });
        $('inventory-count').textContent = `${items.length} itens · inventário de ${HEROES[hero].name}`;
        $('inventory-list').innerHTML = items.length ? items.map((i) => { const d = i.definition; return `<article class="item-card tier-${d.tier}"><small>${slots[d.slot]} · ${HEROES[d.hero].name}</small><strong>${esc(d.name)}</strong><p>${Object.entries(d.attributes).map(([k, v]) => `+${v} ${attr[k]}`).join(' · ')}</p><div class="item-actions"><button class="mini-primary" data-equip="${esc(i.id)}" ${i.listed || d.hero !== hero || owned.level < d.requiredLevel ? 'disabled' : ''}>${i.equipped ? 'Desequipar' : d.hero !== hero ? 'Outra classe' : 'Equipar'}</button>${i.equipped ? '' : `<button class="mini-button" data-sell="${esc(i.id)}" ${i.listed ? 'disabled' : ''}>${i.listed ? 'À venda' : 'Vender'}</button>`}</div></article>`; }).join('') : '<p class="empty-state">Conclua um capítulo para receber seu primeiro equipamento.</p>';
        $('inventory-list').querySelectorAll('[data-equip]').forEach(b => b.onclick = () => void this.action(async () => { const item = items.find((i) => i.id === b.dataset.equip); await this.request('equip', { hero, item: item.id, equip: !item.equipped }); await this.hub.refresh(false); }, b));
        $('inventory-list').querySelectorAll('[data-sell]').forEach(b => b.onclick = () => this.sellDialog(items.find((i) => i.id === b.dataset.sell)));
    }
    sellDialog(item) {
        const dialog = $('trade-dialog');
        $('trade-content').innerHTML = `<span class="eyebrow">MERCADO</span><h2>Vender equipamento</h2><p>${esc(item.definition.name)}</p><label>Preço em gold<input id="trade-price" type="number" min="1" max="1000000000" step="1" required value="50" /></label><p>O item fica reservado até a venda ou o cancelamento do anúncio.</p><p id="trade-error" role="status"></p><div class="item-actions"><button type="button" id="trade-cancel" class="outline-button">Cancelar</button><button id="trade-confirm" class="primary-button">ANUNCIAR</button></div>`;
        ...
        $('trade-content').onsubmit = async (e) => { e.preventDefault(); const b = $('trade-confirm'); b.disabled = true; try {
            await this.request('market/sell', { item: item.id, price: Number($('trade-price').value) });
            dialog.close();
    ...
    async loadChat(clear = true) {
        ...
        this.renderRecipients();
        const disabled = channel === 'clan' || channel === 'friends' && !target;
        $('chat-input').disabled = disabled;
        $('chat-form').querySelector('button').disabled = disabled;
        if (disabled) {
            $('chat-messages').innerHTML = `<p class="empty-state">${channel === 'clan' ? 'O chat de guilda estará disponível quando as guildas chegarem.' : 'Selecione um amigo para iniciar uma conversa privada.'}</p>`;
            return;
        }
        if (clear)
    ...
    async loadHistory() { const epoch = ++this.historyEpoch, id = this.accountId; $('history-content').textContent = 'Carregando partidas…'; await this.action(async () => { const data = await this.request(`history?page=${this.page}&mode=${$('history-mode').value}`); if (epoch !== this.historyEpoch || id !== this.accountId)
        return; $('history-content').innerHTML = data.matches.length ? `<table><thead><tr><th>Data</th><th>Modo</th><th>Fase</th><th>Herói</th><th>Resultado</th><th>Tempo</th><th>Gold</th></tr></thead><tbody>${data.matches.map((m) => `<tr><td>${date(m.created_at)}</td><td>${modeNames[m.mode]}</td><td>${CHAPTERS[m.chapter].name}</td><td>${HEROES[m.hero].name}</td><td>${outcomes[m.player_outcome]}</td><td>${duration(m.duration_ms)}</td><td>+${m.gold}</td></tr>`).join('')}</tbody></table>` : '<p class="empty-state">Suas partidas concluídas aparecerão aqui.</p>'; this.pager('history-pages', data.page, data.hasMore, p => { this.page = p; void this.loadHistory(); }); }); }
    async loadRanking() { const epoch = ++this.rankingEpoch, mode = $('ranking-mode').value; $('ranking-chapter').disabled = mode === 'pvp'; $('ranking-content').textContent = 'Carregando classificação…'; await this.action(async () => { const data = await this.request(`ranking?mode=${mode}&chapter=${$('ranking-chapter').value}`); if (epoch !== this.rankingEpoch)
        return; $('ranking-reset').textContent = `Semana atual · próxima virada: ${date(data.nextReset)} (horário do seu navegador)`; $('ranking-content').innerHTML = data.entries.length ? `<table><thead><tr><th>#</th><th>${mode === 'coop' ? 'Equipe' : 'Jogador'}</th>${mode === 'pvp' ? '<th>Abates</th>' : '<th>Fase</th><th>Melhor tempo</th>'}</tr></thead><tbody>${data.entries.map((m, i) => `<tr><td>${i + 1}</td><td>${esc(m.name)}</td>${mode === 'pvp' ? `<td>${m.kills}</td>` : `<td>${CHAPTERS[m.chapter].name}</td><td>${duration(m.duration_ms)}</td>`}</tr>`).join('')}</tbody></table>` : '<p class="empty-state">Ainda não há resultados nesta semana.</p>'; }); }
    async loadMarket() { const epoch = ++this.marketEpoch, id = this.accountId; $('market-content').textContent = 'Carregando anúncios…'; await this.action(async () => { const q = $('market-query').value, slot = $('market-slot').value, mine = $('market-mine').checked; const data = await this.request(`market?page=${this.marketPage}&q=${encodeURIComponent(q)}&slot=${slot}${mine ? '&mine=1' : ''}`); if (epoch !== this.marketEpoch || id !== this.accountId)
        return; $('market-content').innerHTML = data.listings.length ? data.listings.map((l) => `<article class="item-card tier-${l.definition.tier}"><small>${slots[l.definition.slot]} · ${HEROES[l.definition.hero].name}</small><strong>${esc(l.definition.name)}</strong><p>${Object.entries(l.definition.attributes).map(([k, v]) => `+${v} ${attr[k]}`).join(' · ')}</p><small>Vendedor: ${esc(l.seller_name)}</small><div class="item-actions"><b>✦ ${l.price} gold</b><button class="mini-primary" data-listing="${esc(l.id)}">${l.seller === this.accountId ? 'Cancelar anúncio' : 'Comprar'}</button></div></article>`).join('') : '<p class="empty-state">Nenhum anúncio encontrado. Você pode vender itens pelo inventário do herói.</p>'; $('market-content').querySelectorAll('[data-listing]').forEach(b => b.onclick = () => { const listing = data.listings.find((l) => l.id === b.dataset.listing); if (listing.seller === this.accountId)
