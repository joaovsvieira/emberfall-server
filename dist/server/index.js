...
var index_default = { async fetch(request, env) {
  ...
  if (url.pathname === "/internal/data") {
    ...
    try {
      ...
      if (body.length > 2e4) return json({ error: "Payload too large" }, 413);
      ...
      return json({ error: /UNIQUE constraint/i.test(e?.message ?? "") ? "conflict" : "storage_unavailable" }, /UNIQUE constraint/i.test(e?.message ?? "") ? 409 : 503);
    }
  }
