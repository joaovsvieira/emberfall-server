...
async function gameData(db, op, v) {
  ...
  const validHero = (hero) => {
    if (!HERO_IDS.includes(hero)) fail("Her\xF3i inv\xE1lido.");
  };
  ...
  if (op === "game-profile") {
    ...
    return { gold: account?.gold ?? 0, heroes: HERO_IDS.map((id) => {
      const xp = heroes.find((h) => h.hero === id)?.xp ?? 0;
      const inventory = items.filter((i) => i.hero === id);
      return { id, ...progression(xp), attributes: attributes(id, xp, inventory.filter((i) => i.equipped)), inventory: inventory.map(itemView) };
    }) };
  }
  if (op === "equip") {
    validHero(v.hero);
    if (typeof v.equip !== "boolean") fail("A\xE7\xE3o inv\xE1lida.");
    const item = await one("SELECT * FROM items WHERE id=? AND account_id=? AND hero=?", v.item, v.id, v.hero), def = ITEMS[item?.catalog_id];
    if (!item || !def) fail("Item n\xE3o encontrado.", 404);
    if (v.equip) {
      if (def.hero !== v.hero) fail("Este item pertence a outra classe.");
      const h = await one("SELECT xp FROM hero_progress WHERE account_id=? AND hero=?", v.id, v.hero);
      if (progression(h?.xp ?? 0).level < def.requiredLevel) fail("N\xEDvel insuficiente.");
    }
  ...
  if (op === "rewards" || op === "settle") {
    ...
    if (![1, 2, 3].includes(v.chapter) || !["solo", "coop", "pvp"].includes(v.mode) || !Array.isArray(v.players) || !v.players.length || v.players.length > 4 || !Array.isArray(v.kills) || v.kills.length > 100) fail("Partida inv\xE1lida.");
    const now = v.finishedAt ?? Date.now(), queries = [];
    if (op === "settle") queries.push(stmt("INSERT OR IGNORE INTO matches(id,mode,chapter,duration_ms,outcome,week,created_at) VALUES(?,?,?,?,?,?,?)", v.round, v.mode, v.chapter, Math.max(0, Math.round(v.duration * 1e3)), v.outcome, weekStart(now), now));
    for (const p of v.players) {
      ...
      if (won) {
        ...
        if (!def || def.tier !== v.chapter) fail("Loot inv\xE1lido.");
  ...
  if (op === "friend-action") {
    if (v.id === v.target) fail("Escolha outro jogador.");
    const [a, b] = pair(v.id, String(v.target ?? ""));
    if (!await one("SELECT id FROM accounts WHERE id=?", v.target)) fail("Jogador n\xE3o encontrado.", 404);
    ...
    else fail("A\xE7\xE3o inv\xE1lida.");
    return { ok: true };
  }
  if (op === "chat-read" || op === "chat-send") {
    if (v.channel === "clan") fail("Guildas ainda n\xE3o est\xE3o dispon\xEDveis.");
    if (!["global", "friends"].includes(v.channel)) fail("Canal inv\xE1lido.");
    ...
    if (op === "chat-send") {
      ...
      if (!body || body.length > 500) fail("Envie de 1 a 500 caracteres.");
      const recent = await one("SELECT COUNT(*) AS n FROM chat_messages WHERE sender=? AND created_at>?", v.id, Date.now() - 1e4);
      if (recent.n >= 5) fail("Aguarde um momento antes de enviar mais mensagens.", 429);
      const inserted = await run("INSERT INTO chat_messages(sender,recipient,body,created_at) SELECT ?,?,?,? WHERE (SELECT COUNT(*) FROM chat_messages WHERE sender=? AND created_at>?)<5", v.id, recipient, body, Date.now(), v.id, Date.now() - 1e4);
      if (inserted.meta.changes === 0) fail("Aguarde um momento antes de enviar mais mensagens.", 429);
    }
  ...
  if (op === "market-sell") {
    if (!Number.isSafeInteger(v.price) || v.price < 1 || v.price > 1e9) fail("Informe um pre\xE7o inteiro entre 1 e 1 bilh\xE3o.");
    ...
    if (result.meta.changes === 0) fail("Este item n\xE3o est\xE1 dispon\xEDvel para venda.", 409);
    return { ok: true };
  }
  if (op === "market-cancel") {
  ...
  if (op === "market-buy") {
    ...
    if (result[0].meta.changes === 0) fail("Item vendido, saldo insuficiente ou compra inv\xE1lida.", 409);
    return { ok: true };
  }
  if (op === "history") {
