...
export class DataStore {
 ...
 async call(op,value){
  ...
  if(!r.ok){const data=await r.json().catch(()=>({}));throw Object.assign(new Error(data.error&&data.error!=='storage_unavailable'?data.error:'Não foi possível acessar os dados. Tente novamente.'),{status:r.status});}
...
export class Accounts {
 ...
 async authenticate(options){
  ...
  const a=await this.store.call('session',{hash:data.hash});if(!a)throw new Error('Sua sessão expirou. Faça login novamente.');return this.enrich(a);
 }
 async complete(ids,chapter,round,payload){return payload?this.store.call('settle',payload):this.store.call('complete',{ids,chapter,round});}
 async enrich(a){return {...this.profile(a),...await this.store.call('game-profile',{id:a.id})};}
 async getProfile(id){const a=await this.store.call('account',{id});return a?await this.enrich(a):null;}
}
export function tokenOf(req){const value=(req.headers.cookie??'').split(';').map(v=>v.trim()).find(v=>v.startsWith('emberfall_session='));return value?.slice('emberfall_session='.length)??'';}
export function accountRoutes(app,accounts){
 ...
 for(const action of ['login','register'])app.post('/api/'+action,route(async(req,res)=>{
  ...
  activeHashes++;try{const a=await accounts.credentials(req.body?.username,req.body?.password,action==='register');const previous=tokenOf(req);const token=await accounts.createSession(a.id);await accounts.logout(previous);cookie(res,token);res.json({profile:await accounts.enrich(a)});}finally{activeHashes--;}
 }));
 app.get('/api/me',route(async(req,res)=>{const a=await accounts.session(tokenOf(req));if(!a)return res.status(401).json({error:'Faça login para continuar.'});res.json({profile:await accounts.enrich(a)});}));
