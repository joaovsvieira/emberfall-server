...
const themes=['','da Floresta','de Obsidiana','da Geada','das Dunas','do Abismo','do Firmamento'];
export const ITEMS=Object.fromEntries(CHAPTER_IDS.flatMap(tier=>HERO_IDS.flatMap(hero=>Object.keys(SLOTS).map(slot=>{
...
// Power v1: permanent offensive and effective-health attributes; temporary buffs and skins excluded.
export function heroPower(stats){return Math.round(10*Math.sqrt(stats.damage*(stats.attackSpeed/100)*stats.maxHp*(1+stats.defense/100))*(stats.speed/100)**.25);}
