...
export async function startServer(port=Number(process.env.PORT)||2567){
  ...
  const server=new Server({
    ...
    express:app=>{
      ...
      app.get('/health',(_req,res)=>res.json({ok:true,game:'emberfall',protocol:1,modes:['coop','pvp'],heroes:['kael','lyra','aurel','sylva'],chapters:3,maxCoopPlayers:4,release:'0.5.0'}));
