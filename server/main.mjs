...
export async function startServer(port=Number(process.env.PORT)||2567,{accounts=new Accounts()}={}){
  ...
  const server=new Server({
    ...
    express:app=>{
      ...
      app.get('/health',(_req,res)=>res.json({ok:true,game:'emberfall',protocol:1,modes:['solo','coop','pvp'],accounts:true,heroes:['kael','lyra','aurel','sylva'],chapters:3,maxCoopPlayers:4,release:'0.9.0'}));
