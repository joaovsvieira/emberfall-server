...
export class ForestRoom extends Room {
  ...
  onAuth(_client,options){const hero=options?.hero??'kael';if(!Object.hasOwn(HEROES,hero))throw new ServerError(400,'Herói inválido.');return {name:cleanName(options?.name),hero};}
  onJoin(client,_options,auth){
    ...
    this.members.set(client.sessionId,{id:client.sessionId,name:auth.name,hero:auth.hero,ready:false,connected:true,paused:false,index,lastSeq:0,ack:0,queue:[],input:{},lastSeen:Date.now()});
