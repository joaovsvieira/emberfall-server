...
export class ForestRoom extends Room {
  ...
  async onCreate(){
    ...
    this.onMessage('pause',(client,value)=>{const m=this.members.get(client.sessionId);if(m&&typeof value==='boolean'){m.paused=value;m.input={};m.queue=[];m.ack=m.lastSeq;this.sendLobby();}});
    this.onMessage('restart',(client)=>{if(client.sessionId!==this.host||!['dead','won'].includes(this.world.status))return;if(this.members.size<2){client.send('notice','Volte ao menu para criar uma nova sala com seu companheiro.');return;}this.beginRound();});
    ...
    this.clock.setTimeout(()=>this.disconnect(),2*60*60*1000);
  }
  ...
  onDrop(client){const m=this.members.get(client.sessionId);if(m){m.connected=false;m.input={};m.queue=[];m.ack=m.lastSeq;}this.allowReconnection(client,25);this.sendLobby();}
  ...
  onLeave(client){
    this.members.delete(client.sessionId);this.world.players=this.world.players.filter(p=>p.id!==client.sessionId);
    ...
    this.sendLobby();
    if(this.members.size&&this.stage==='playing')this.broadcast('notice','Seu companheiro saiu. Você pode concluir a fase ou voltar ao menu.');
  }
  onDispose(){codes.delete(this.roomId);}
  isPaused(){return [...this.members.values()].some(m=>m.paused||!m.connected);}
  lobby(){return {code:this.roomId,host:this.host,stage:this.stage,round:this.round,countdown:this.stage==='countdown'?Math.max(0,Math.ceil((this.countdownAt-Date.now())/1000)):0,paused:this.isPaused(),members:[...this.members.values()].map(({id,name,ready,connected,paused,index})=>({id,name,ready,connected,paused,index}))};}
  sendLobby(){this.broadcast('lobby',this.lobby());}
  beginRound(){
    this.lock();this.world=new World();this.round++;this.world.players=[...this.members.values()].map((m,i)=>{m.input={};m.queue=[];m.ack=m.lastSeq;m.paused=false;return createPlayer(m.id,m.name,190+i*85);});
  ...
  advance(delta){
    ...
    if(this.stage==='playing'&&!this.isPaused()&&this.world.status==='playing'){
      ...
      let count=0;while(this.accumulator>=1000/60&&count++<6){
        ...
        this.world.updateCoop(1/60,inputs);this.accumulator-=1000/60;
      }
