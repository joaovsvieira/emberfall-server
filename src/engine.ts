...
export class World {
 ...
 stepPlayer(dt:number,input:Input={}){const p=this.player;if(p.hp<=0)return;
   ...
   if(p.x>3890&&!this.bossActive&&this.players.filter(p=>p.hp>0).every(p=>p.x>3780)){this.bossActive=true;for(const member of this.players){member.checkpoint=3930;member.hp=member.hp>0?Math.min(100,member.hp+35):0;}this.emit('boss');this.emit('toast',{text:'Guardião Ancestral · Pule sobre as ondas e use o dash para esquivar.'});}
 }
 stepEnemies(dt:number){
   for(const e of this.enemies){if(e.dead)continue;const target=this.players.filter(p=>p.hp>0).sort((a,b)=>Math.abs(a.x-e.x)-Math.abs(b.x-e.x))[0];if(!target)continue;this.player=target;const p=target;e.flash=Math.max(0,e.flash-dt);e.knock*=Math.max(0,1-dt*10);e.x+=e.knock*dt;const dx=p.x-e.x,dy=Math.abs(p.y-e.y);e.active=e.kind==='boss'?this.bossActive:Math.abs(dx)<660;if(!e.active)continue;e.dir=dx>=0?1:-1;e.timer-=dt;
     if(e.windup>0){e.windup-=dt;if(e.windup<=0){if(e.kind==='boss'){if(e.phase%2===0){this.emit('slam',{x:e.attackX,y:615});for(const member of this.players){if(member.hp>0&&Math.abs(member.x-e.attackX)<145&&member.y>470){this.player=member;this.damagePlayer(24,e.x);}}this.player=p;for(const dir of [-1,1])this.projectiles.push({x:e.attackX,y:594,vx:dir*310,vy:0,life:2.1,friendly:false,hits:new Set()});}else{for(let n=-1;n<=1;n++){const angle=Math.atan2(p.y-55-(e.y-140),p.x-e.x)+n*.26;this.projectiles.push({x:e.x,y:e.y-140,vx:Math.cos(angle)*310,vy:Math.sin(angle)*310,life:2.8,friendly:false,hits:new Set()});}this.emit('enemyShot',{x:e.x,y:e.y-140});}e.timer=e.hp<220?1.0:1.65;e.phase++;}else{if(Math.abs(dx)<105&&dy<110)this.damagePlayer(e.kind==='bat'?9:13,e.x);this.emit('enemySwipe',{x:e.x+e.dir*35,y:e.y-35,dir:e.dir});e.timer=1.25;}}
     }else if(e.kind==='boss'){if(e.timer<=0){e.windup=.85;e.attackX=p.x;this.emit('warning',{x:e.attackX,y:615,value:.85,kind:e.phase%2===0?'slam':'shot'});}else if(Math.abs(dx)>175)e.x+=e.dir*50*dt;e.x=Math.max(3980,Math.min(4650,e.x));
     ...
     }else{if(e.kind==='bat'){e.y+=(Math.max(380,p.y-40)-e.y)*dt*1.5; if(Math.abs(dx)>60)e.x+=e.dir*115*dt;}else if(Math.abs(dx)>64){const next=e.x+e.dir*78*dt;const ground=platforms.find(b=>b.ground&&next>b.x+28&&next<b.x+b.w-28);if(ground)e.x=next;}if(Math.abs(dx)<90&&dy<105&&e.timer<=0){e.windup=.42;this.emit('warning',{x:e.x,y:e.y-80,value:.42,kind:'melee'});}}
   }
 }
