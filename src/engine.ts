...
export const HEROES={kael:{name:'Kael',role:'Espadachim',attack:'Combo de espada',skill1:'Corte solar',skill2:'Nova rúnica'},lyra:{name:'Lyra',role:'Maga das brasas',attack:'Bolas de fogo',skill1:'Orbe incandescente',skill2:'Explosão ígnea'}};
...
export type Enemy = {id:number;kind:string;x:number;y:number;home:number;baseY:number;hp:number;maxHp:number;dir:number;timer:number;windup:number;flash:number;knock:number;phase:number;active:boolean;dead:boolean;attackX:number};
...
export class World {
 ...
 constructor(chapter:ChapterId=1){
   ...
   if(this.chapter===2)for(const e of this.enemies){e.hp=e.maxHp=Math.round(e.maxHp*(e.kind==='boss'?1.5:1.35));e.timer*=.8;}
 }
 ...
 damagePlayer(amount:number,sourceX:number){const p=this.player;if(p.invincible>0||this.status!=='playing')return;p.hp=Math.max(0,p.hp-amount);p.invincible=this.mode==='pvp'?.22:1.15;p.vx=(p.x>=sourceX?1:-1)*240;this.hitCount=0;this.emit('hurt',{x:p.x,y:p.y-65,value:amount});if(p.hp<=0){p.vx=0;p.vy=0;p.y=this.platforms.filter(b=>p.x>b.x&&p.x<b.x+b.w&&b.y>=p.y).sort((a,b)=>a.y-b.y)[0]?.y??615;if(this.mode==='pvp'){this.winnerId=this.players.find(q=>q.id!==p.id&&q.hp>0)?.id??null;this.status='won';this.emit('won');return;}this.emit('downed');if(this.players.every(p=>p.hp<=0)){this.status='dead';this.emit('dead');}}}
 ...
 action(action:Action){if(this.status!=='playing'||this.player.hp<=0)return;const p=this.player;
   ...
   if(action==='skill2'&&p.skill2<=0){p.skill2=10;p.attack=.45;p.invincible=Math.max(p.invincible,.5);this.emit(p.hero==='lyra'?'inferno':'nova',{x:p.x,y:p.y-45});for(const e of this.enemies){if(!e.dead&&Math.hypot(e.x-p.x,e.y-p.y)<285)this.hitEnemy(e,p.hero==='lyra'?64:76,e.x>p.x?1:-1);}for(const target of this.players)if(Math.hypot(target.x-p.x,target.y-p.y)<285)this.hitOpponent(target,p.hero==='lyra'?28:32);}
 }
 ...
 stepPlayer(dt:number,input:Input={}){const p=this.player;if(p.hp<=0)return;
   for(const key of ['invincible','dash','dashCooldown','attack','attackCooldown','skill1','skill2','jumpBuffer','coyote'] as const)p[key]=Math.max(0,p[key]-dt);
   ...
   if(p.y>850){p.invincible=0;this.damagePlayer(this.chapter===2?24:16,p.x);p.x=this.checkpoint;p.y=p.hp>0?430:615;p.vy=0;p.vx=0;p.jumps=0;this.emit('toast',{text:'Cuidado com os abismos. Use o pulo duplo e o dash.'});}
 ...
 stepEnemies(dt:number){
   for(const e of this.enemies){if(e.dead)continue;const target=this.players.filter(p=>p.hp>0).sort((a,b)=>Math.abs(a.x-e.x)-Math.abs(b.x-e.x))[0];if(!target)continue;this.player=target;const p=target;e.flash=Math.max(0,e.flash-dt);e.knock*=Math.max(0,1-dt*10);e.x+=e.knock*dt;const dx=p.x-e.x,dy=Math.abs(p.y-e.y);e.active=e.kind==='boss'?this.bossActive:Math.abs(dx)<660;if(!e.active)continue;e.dir=dx>=0?1:-1;e.timer-=dt*(this.chapter===2?1.25:1);
     if(e.windup>0){e.windup-=dt;if(e.windup<=0){if(e.kind==='boss'){if(e.phase%2===0){this.emit('slam',{x:e.attackX,y:615});for(const member of this.players){if(member.hp>0&&Math.abs(member.x-e.attackX)<145&&member.y>470){this.player=member;this.damagePlayer(this.chapter===2?32:24,e.x);}}this.player=p;for(const dir of [-1,1])this.projectiles.push({x:e.attackX,y:594,vx:dir*(this.chapter===2?390:310),vy:0,life:2.1,friendly:false,hits:new Set()});}else{for(let n=this.chapter===2?-2:-1;n<=(this.chapter===2?2:1);n++){const angle=Math.atan2(p.y-55-(e.y-140),p.x-e.x)+n*.26;this.projectiles.push({x:e.x,y:e.y-140,vx:Math.cos(angle)*310,vy:Math.sin(angle)*310,life:2.8,friendly:false,hits:new Set()});}this.emit('enemyShot',{x:e.x,y:e.y-140});}e.timer=e.hp<e.maxHp*.46?1.0:1.65;e.phase++;}else{if(Math.abs(dx)<105&&dy<110)this.damagePlayer((e.kind==='bat'?9:13)*(this.chapter===2?1.3:1),e.x);this.emit('enemySwipe',{x:e.x+e.dir*35,y:e.y-35,dir:e.dir});e.timer=1.25;}}
     }else if(e.kind==='boss'){if(e.timer<=0){e.windup=this.chapter===2?.7:.85;e.attackX=p.x;this.emit('warning',{x:e.attackX,y:615,value:.85,kind:e.phase%2===0?'slam':'shot'});}else if(Math.abs(dx)>175)e.x+=e.dir*50*dt;e.x=Math.max(3980,Math.min(4650,e.x));
     }else if(e.kind==='wraith'){e.y=e.baseY+Math.sin(this.time*2+e.id)*18;if(Math.abs(dx)<500&&e.timer<=0){const angle=Math.atan2(p.y-50-e.y,p.x-e.x);this.projectiles.push({x:e.x,y:e.y-35,vx:Math.cos(angle)*235,vy:Math.sin(angle)*235,life:3,friendly:false,hits:new Set()});e.timer=2.6;this.emit('enemyShot',{x:e.x,y:e.y-35});}
     }else{if(e.kind==='bat'){e.y+=(Math.max(380,p.y-40)-e.y)*dt*1.5; if(Math.abs(dx)>60)e.x+=e.dir*(this.chapter===2?140:115)*dt;}else if(Math.abs(dx)>64){const next=e.x+e.dir*(this.chapter===2?100:78)*dt;const ground=this.platforms.find(b=>b.ground&&next>b.x+28&&next<b.x+b.w-28);if(ground)e.x=next;}if(Math.abs(dx)<90&&dy<105&&e.timer<=0){e.windup=.42;this.emit('warning',{x:e.x,y:e.y-80,value:.42,kind:'melee'});}}
   }
 }
 stepProjectiles(dt:number){
   for(const s of this.projectiles){s.x+=s.vx*dt;s.y+=s.vy*dt;s.life-=dt;if(s.friendly){this.player=this.players.find(p=>p.id===s.owner)??this.players[0];if(!this.player)continue;if(this.mode==='pvp')for(const target of this.players){if(target.id!==s.owner&&target.hp>0&&!s.hits.has(target.id)&&Math.abs(s.x-target.x)<45&&Math.abs(s.y-(target.y-50))<65){s.hits.add(target.id);this.hitOpponent(target,s.pvpDamage??26);if(s.kind==='fireball'){s.life=0;break;}}}if(s.life>0)for(const e of this.enemies){if(!e.dead&&!s.hits.has(e.id)&&Math.abs(s.x-e.x)<(e.kind==='boss'?100:55)&&Math.abs(s.y-(e.y-(e.kind==='boss'?100:40)))<110){s.hits.add(e.id);this.hitEnemy(e,s.damage??58,Math.sign(s.vx));if(s.kind==='fireball'){s.life=0;break;}}}}else for(const p of this.players){if(p.hp>0&&Math.abs(s.x-p.x)<25&&Math.abs(s.y-(p.y-45))<48){this.player=p;this.damagePlayer(this.chapter===2?16:12,s.x);s.life=0;break;}}}
   this.projectiles=this.projectiles.filter(s=>s.life>0);
 }
 get hazards(){return this.chapter===2&&this.mode!=='pvp'?[{x:1810,offset:0},{x:2720,offset:1.7},{x:3520,offset:3.2}]:[];}
 hazardState(offset:number){const phase=(this.time+offset)%5;return phase>=4.2?'active':phase>=3.2?'warning':'idle';}
 stepHazards(){if(this.predicting||this.status!=='playing')return;for(const h of this.hazards){if(this.hazardState(h.offset)!=='active')continue;for(const p of this.players){if(p.hp>0&&Math.abs(p.x-h.x)<55&&p.y>480){this.player=p;this.damagePlayer(18,h.x);}}}}
