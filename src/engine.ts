export type HeroId='kael'|'lyra'|'aurel'|'sylva'|'nyxar';
...
export type Input = {left?:boolean;right?:boolean;down?:boolean;attack?:boolean;actions?:Action[]};
...
function expeditionPlatforms(offset:number):Platform[]{return [
...
export const SPECIES:Record<number,Record<string,string>>={1:{bat:'forest_dragon'},2:{goblin:'lava_imp',bat:'ashwing',wraith:'ember_mage',boss:'obsidian_guardian'},3:{goblin:'frost_reaver',bat:'ice_wyvern',wraith:'frost_sorcerer',boss:'ice_dragon'},4:{goblin:'sand_scorpion',bat:'sand_scarab',wraith:'sand_priest',boss:'sand_pharaoh'},5:{goblin:'coral_guard',bat:'abyss_ray',wraith:'abyss_siren',boss:'leviathan'},6:{goblin:'storm_knight',bat:'harpy',wraith:'cloud_seer',boss:'storm_titan'}};
...
export class World {
 ...
 constructor(chapter:ChapterId=1){
   ...
   if(this.chapter>=4){specs.length=0;for(const g of this.platforms.filter(b=>b.ground&&b.x<3800)){const count=this.chapter+1;for(let i=0;i<count;i++){const x=g.x+Math.max(170,g.w*(i+1)/(count+1));if(x>3800)continue;const kind=i%3===0?'goblin':i%3===1?'bat':'wraith';specs.push([kind,x,kind==='goblin'?615:465-(i%2)*25]);}}specs.push(['wraith',2300,460],['bat',3110,440],['boss',4390,615]);}
   ...
   if(this.chapter>1)for(const e of this.enemies){e.hp=e.maxHp=Math.round(e.maxHp*(e.kind==='boss'?[1,1,1.5,2.15,2.9,3.85,5][this.chapter]:[1,1,1.35,1.75,2.25,2.85,3.6][this.chapter]));e.timer*=this.chapter===2?.8:this.chapter===3?.65:Math.max(.4,.7-(this.chapter-3)*.08);}
 }
 ...
 action(action:Action){if(this.status!=='playing'||this.player.hp<=0)return;const p=this.player;
   ...
   if(p.hero==='nyxar'){
     if(action==='skill2'&&p.skill2<=0){p.skill2=40;p.demonTime=15;p.attack=.4;this.emit('metamorphosis',{x:p.x,y:p.y-60});return;}
     if(action==='attack'&&p.attackCooldown<=0){p.attack=.3;p.attackCooldown=p.demonTime>0?.4:.45;if(p.demonTime>0){this.emit('voidClaw',{x:p.x,y:p.y-58,dir:p.dir});for(const e of this.enemies){const dx=e.x-p.x;if(!e.dead&&dx*p.dir>-30&&Math.abs(dx)<(e.kind==='boss'?180:150)&&Math.abs(e.y-p.y)<120)this.hitEnemy(e,36,p.dir);}for(const t of this.players){const dx=t.x-p.x;if(dx*p.dir>-30&&Math.abs(dx)<150&&Math.abs(t.y-p.y)<120)this.hitOpponent(t,18);}}else this.castBolt('void',23,12,660,1.15);return;}
     if(action==='skill1'&&p.skill1<=0){p.skill1=8;p.attack=.4;this.castBolt('rift',54,25,780,1.1,true);return;}
   }
   if(p.hero==='aurel'){
 ...
 castBolt(kind:string,damage:number,pvpDamage:number,speed:number,life:number,pierce=false){const p=this.player;this.projectiles.push({x:p.x+p.dir*42,y:p.y-62,vx:p.dir*speed,vy:0,life,friendly:true,owner:p.id,kind,damage,pvpDamage,pierce,hits:new Set()});this.emit(kind==='void'||kind==='rift'?'voidBolt':kind==='light'?'lightBolt':'arrow',{x:p.x+p.dir*42,y:p.y-62,dir:p.dir});}
 ...
 updateCoop(dt:number,inputs:Record<string,Input>={}){
   ...
   if(this.mode!=='pvp')for(const p of this.players){if(p.hp>0)continue;p.revive+=dt;if(p.revive>=3){this.player=p;p.hp=p.maxHp;p.x=this.bossActive?3980:p.checkpoint;p.y=430;p.vx=p.vy=0;p.jumps=0;p.dropping=false;p.demonTime=0;p.jumpBuffer=0;p.attack=0;p.dash=0;p.invincible=2;p.revive=0;this.emit('revived',{x:p.x,y:p.y-90});}}
   this.player=selected;
 }
 stepPlayer(dt:number,input:Input={}){const p=this.player;if(p.hp<=0)return;
   for(const key of ['invincible','dash','dashCooldown','attack','attackCooldown','skill1','skill2','jumpBuffer','coyote','shieldTime','demonTime'] as const)p[key]=Math.max(0,p[key]-dt*(key==='attackCooldown'?p.attackSpeed/100:1));
   if(p.shieldTime<=0)p.shield=0;
   if(input.down&&p.grounded&&this.platforms.some(b=>!b.ground&&Math.abs(p.y-b.y)<3&&p.x+17>b.x&&p.x-17<b.x+b.w)){p.dropping=true;p.grounded=false;p.coyote=0;p.jumpBuffer=0;p.vy=Math.max(90,p.vy);}
   ...
   if(p.vy>=0)for(const b of this.platforms){if((!p.dropping||b.ground)&&p.x+17>b.x&&p.x-17<b.x+b.w&&oldY<=b.y+2&&p.y>=b.y){p.y=b.y;p.vy=0;p.grounded=true;p.dropping=false;p.jumps=0;p.coyote=.1;break;}}
   if(p.jumpBuffer>0)this.tryJump();
   if(p.y>850){p.invincible=0;this.damagePlayer(this.chapter>=3?30:this.chapter===2?24:16,p.x);p.x=this.checkpoint;p.y=p.hp>0?430:615;p.vy=0;p.vx=0;p.jumps=0;p.dropping=false;this.emit('toast',{text:'Cuidado com os abismos. Use o pulo duplo e o dash.'});}
