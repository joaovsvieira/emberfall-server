...
export class Hub {
 profile:any=null;chapter=1;tab='heroes';authMode='login';busy=false;generation=0;heroSave=Promise.resolve();
 constructor(private scene:any){
  for(const tab of ['heroes','map','settings'])$('tab-'+tab).onclick=()=>this.setTab(tab);
 ...
 async refresh(showLogin=true){const generation=this.generation;try{const data=await this.api('me');if(generation===this.generation)this.accept(data.profile,false);}catch(e:any){if(generation!==this.generation)return;if(e.status===401){this.profile=null;visible('auth-screen',true);visible('start-screen',false);$('auth-status').textContent='Entre para continuar sua jornada.';}else if(showLogin){visible('auth-screen',true);visible('start-screen',false);$('auth-status').textContent=e.message;}else $('hub-status').textContent='Não foi possível atualizar seu progresso. Tente novamente ao voltar ao menu.';}}
 accept(profile:any,initial=true){const first=!this.profile;this.profile=profile;visible('auth-screen',false);$('account-name').textContent=profile.name;($('mp-name') as HTMLInputElement).value=profile.name;($('mp-name') as HTMLInputElement).readOnly=true;
  if(first||initial)this.scene.selectHero(profile.preferredHero);this.renderHeroes();this.renderMap();if(this.scene.session==='menu'){visible('start-screen',true);this.setTab(this.tab);}if(first){const invite=new URL(location.href);if(invite.searchParams.has('room')){$(invite.searchParams.get('mode')==='pvp'?'pvp':'multiplayer').click();($('mp-code') as HTMLInputElement).value=invite.searchParams.get('room')??'';}}
 }
 setTab(tab:string){this.tab=tab;for(const name of ['heroes','map','settings']){visible('panel-'+name,name===tab);$('tab-'+name).setAttribute('aria-selected',String(name===tab));}this.syncSound();this.renderHeroes();this.renderMap();}
 syncSound(){$('settings-sound').textContent=$('sound').getAttribute('aria-label')==='Desativar som'?'Som: ativado':'Som: desativado';}
 heroSelected(hero:HeroId){if(!this.profile)return;this.profile.preferredHero=hero;this.renderHeroes();this.heroSave=this.heroSave.catch(()=>{}).then(async()=>{try{await this.api('hero',{hero});}catch(e:any){$('hub-status').textContent='Não foi possível salvar o herói padrão: '+e.message;}});}
 ...
 async logout(){if(this.busy)return;this.busy=true;this.generation++;($('settings-logout') as HTMLButtonElement).disabled=true;try{await this.heroSave;await this.api('logout',{});this.profile=null;await this.scene.goToMenu();visible('start-screen',false);visible('mp-panel',false);visible('auth-screen',true);this.renderAuth();$('auth-username').focus();}catch(e:any){$('hub-status').textContent=e.message;}finally{this.busy=false;($('settings-logout') as HTMLButtonElement).disabled=false;}}
}
