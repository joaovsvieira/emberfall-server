...
export class Hub {
 ...
 constructor(public scene:any){
  for(const tab of ['shop','heroes','map','history','market','ranking','clan','settings'])$('tab-'+tab).onclick=()=>this.setTab(tab);
  ...
  this.features=new Features(this);this.expansion=new Expansion(this);
 ...
 accept(profile:any,initial=true){const first=!this.profile;this.profile=profile;if(first||initial)this.tab='map';this.setAuthChrome(false);visible('auth-screen',false);$('account-name').textContent=profile.name;$('account-id').textContent='ID da conta: '+profile.id;($('mp-name') as HTMLInputElement).value=profile.name;($('mp-name') as HTMLInputElement).readOnly=true;
  if(first||initial)this.scene.selectHero(profile.preferredHero);this.features.accepted();this.expansion.accepted();this.renderHeroes();this.renderMap();this.renderFooter();if(this.scene.resultShown&&this.scene.world.mode!=='pvp')this.scene.renderLoot();if(this.scene.session==='menu'){visible('start-screen',true);if(first||initial)this.setTab(this.tab);}if(first){const invite=new URL(location.href);if(invite.searchParams.has('room')){$(invite.searchParams.get('mode')==='pvp'?'pvp':'multiplayer').click();($('mp-code') as HTMLInputElement).value=invite.searchParams.get('room')??'';}}
 }
 setTab(tab:string){this.tab=tab;this.closeWidgets();for(const name of ['shop','heroes','map','history','market','ranking','clan','settings']){visible('panel-'+name,name===tab);$('tab-'+name).setAttribute('aria-selected',String(name===tab));}this.syncSound();this.renderHeroes();this.renderMap();this.renderFooter();this.features.open(tab);this.expansion.open(tab);}
