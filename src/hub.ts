...
export class Hub {
 ...
 constructor(public scene:any){
  for(const tab of ['heroes','map','history','market','ranking','settings'])$('tab-'+tab).onclick=()=>this.setTab(tab);
 ...
 accept(profile:any,initial=true){const first=!this.profile;this.profile=profile;if(first||initial)this.tab='map';this.setAuthChrome(false);visible('auth-screen',false);$('account-name').textContent=profile.name;$('account-id').textContent='ID da conta: '+profile.id;($('mp-name') as HTMLInputElement).value=profile.name;($('mp-name') as HTMLInputElement).readOnly=true;
  if(first||initial)this.scene.selectHero(profile.preferredHero);this.features.accepted();this.renderHeroes();this.renderMap();this.renderFooter();if(this.scene.resultShown&&this.scene.world.mode!=='pvp')this.scene.renderLoot();if(this.scene.session==='menu'){visible('start-screen',true);if(first||initial)this.setTab(this.tab);}if(first){const invite=new URL(location.href);if(invite.searchParams.has('room')){$(invite.searchParams.get('mode')==='pvp'?'pvp':'multiplayer').click();($('mp-code') as HTMLInputElement).value=invite.searchParams.get('room')??'';}}
 }
 setTab(tab:string){this.tab=tab;this.closeWidgets();for(const name of ['heroes','map','history','market','ranking','settings']){visible('panel-'+name,name===tab);$('tab-'+name).setAttribute('aria-selected',String(name===tab));}this.syncSound();this.renderHeroes();this.renderMap();this.renderFooter();this.features.open(tab);}
 syncSound(){$('settings-sound').textContent=$('sound').getAttribute('aria-label')==='Desativar som'?'Som: ativado':'Som: desativado';}
 heroSelected(hero:HeroId){if(!this.profile)return;this.profile.preferredHero=hero;this.renderHeroes();this.renderFooter();this.heroSave=this.heroSave.catch(()=>{}).then(async()=>{try{await this.api('hero',{hero});}catch(e:any){$('hub-status').textContent='Não foi possível salvar o herói padrão: '+e.message;}});}
 renderHeroes(){if(!this.profile)return;const id=this.scene.selectedHero as HeroId,hero=HEROES[id],owned=this.profile.heroes.find((h:any)=>h.id===id);
  $('roster-name').textContent=hero.name;$('roster-role').textContent=hero.role;$('roster-level').textContent=`NÍVEL ${owned?.level??1}`;($('roster-image') as HTMLImageElement).src=this.scene.textures.get(this.scene.heroKey(id,'idle')).getSourceImage().toDataURL();
 ...
 renderMap(){if(!this.profile)return;const unlocked=this.profile.unlockedChapter;
  ...
  for(const id of ['start','multiplayer','pvp'])($(id) as HTMLButtonElement).disabled=this.chapter>unlocked||this.scene.net.connecting;
 }
 ...
 renderFooter(){if(!this.profile)return;const chapter=CHAPTERS[this.chapter as 1|2|3];$('footer-gems').textContent=String(this.profile.gems??0);$('footer-gold').textContent=String(this.profile.gold??0);$('footer-progress').textContent=`CAPÍTULO ${['','I','II','III'][this.chapter]} · ${chapter.name.toUpperCase()}`;const hero=this.scene.selectedHero as HeroId;const meta=HEROES[hero];$('footer-hero-name').textContent=meta.name.toUpperCase();($('footer-hero-image') as HTMLImageElement).src=this.scene.textures.get(this.scene.heroKey(hero,'idle')).getSourceImage().toDataURL();}
 ...
 async logout(){if(this.busy)return;this.busy=true;this.generation++;($('settings-logout') as HTMLButtonElement).disabled=true;try{await this.heroSave;await this.api('logout',{});this.profile=null;this.features?.reset();this.setAuthChrome(true);this.closeWidgets();await this.scene.goToMenu();visible('start-screen',false);visible('mp-panel',false);visible('auth-screen',true);this.renderAuth();$('auth-username').focus();}catch(e:any){$('hub-status').textContent=e.message;}finally{this.busy=false;($('settings-logout') as HTMLButtonElement).disabled=false;}}
}
