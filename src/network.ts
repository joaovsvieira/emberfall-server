...
import {World,type Input,type GameEvent} from './engine.js';
declare global{interface Window{EMBERFALL_MULTIPLAYER_URL?:string;}}
export class Multiplayer {
 room:any=null;client:any=null;snapshot:any=null;lobby:any=null;connected=false;connecting=false;ping=0;seq=0;pending:{seq:number;input:Input}[]=[];predictor=new World();round=-1;leaving=false;lastPing=0;generation=0;
 ...
 async connect(name:string,code=''){
   ...
   try{
     ...
     room.onMessage('snapshot',(value:any)=>this.receive(value));
     room.onMessage('events',(events:GameEvent[])=>this.onEvents(events.filter(e=>!(e.playerId===this.id&&['jump','dash','slash','solar','nova'].includes(e.type)))));
     room.onMessage('notice',(value:string)=>this.onNotice(value));
     room.onMessage('pong',(value:number)=>this.ping=Math.round(performance.now()-value));
     ...
     room.onLeave(()=>{this.connected=false;this.room=null;this.pending=[];if(!this.leaving)this.onConnection('disconnected');});
     room.onError((_code:number,message:string)=>this.onNotice(message||'Não foi possível manter a conexão.'));
 ...
 async leave(){this.leaving=true;this.generation++;this.connecting=false;const room=this.room;this.room=null;this.connected=false;this.pending=[];this.snapshot=null;this.lobby=null;if(room)await room.leave();}
}
