...
test('HTTP login and actual rooms enforce identity, chapter locks, solo victory and chosen PvP map',{timeout:26000},async t=>{
 ...
 room.world.bossActive=true;room.world.hitEnemy(room.world.enemies.at(-1),99999,1);await until(()=>room.progress==='error','surface save failure');roomClient.send('next-chapter');await delay(80);assert.equal(room.chapter,1);assert.equal((await api('me')).body.profile.unlockedChapter,1);rejectSave=false;roomClient.send('retry-progress');await until(()=>room.progress==='saved','persist victory');assert.equal((await api('me')).body.profile.unlockedChapter,2);roomClient.send('next-chapter');await until(()=>room.chapter===2,'advance solo');
 ...
 const duel=await new Client(url).create('forest',{mode:'pvp',chapter:2,hero:'aurel',ticket});rooms.push(duel);observe(duel);await assert.rejects(()=>new Client(url).joinById(duel.roomId,{hero:'kael',ticket:guestTicket}));
 await auth.complete([guest.body.profile.id],1,'guest1');const b=await new Client(url).joinById(duel.roomId,{hero:'kael',ticket:guestTicket});rooms.push(b);observe(b);const d=matchMaker.getLocalRoomById(duel.roomId);d.beginRound();assert.equal(d.world.chapter,2);assert.equal(d.world.mode,'pvp');assert.equal(d.world.enemies.length,0);assert.equal(d.world.hazards.length,0);
