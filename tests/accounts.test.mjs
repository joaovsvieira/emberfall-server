...
test('accounts keep hashed passwords, revocable sessions and per-user chapter unlocks after reopening storage',async t=>{
 ...
 const a=await auth.credentials('Aventureiro','Password123',true),b=await auth.credentials('OutraConta','Password123',true);assert.notEqual(a.password_hash,'Password123');assert.equal(auth.profile(a).unlockedChapter,1);assert.equal(auth.profile(a).heroes.length,5);
