...
test('cooperative chapters retain heroes, require boss completion, and pause for teammate menu exit',{timeout:22000},async t=>{
 ...
 async function startLobby(clients,room){await until(()=>room.stage==='lobby','lobby');for(const c of clients)c.send('ready',true);await until(()=>[...room.members.values()].every(m=>m.ready),'ready again');clients[0].send('start');await until(()=>room.stage==='countdown','countdown');}
 ...
 b.send('next-chapter');await delay(100);assert.equal(room.chapter,1);a.send('next-chapter');await startLobby([a,b],room);await until(()=>sa.snapshot.chapter===2&&sb.snapshot.chapter===2,'chapter transition');
 ...
 room.world.bossActive=true;room.world.hitEnemy(room.world.enemies.at(-1),9999,1);await until(()=>sb.snapshot.status==='won','chapter II won');b.send('next-chapter');await startLobby([b],room);await until(()=>room.chapter===3,'advance to chapter III alone');assert.equal(room.world.players[0].hero,'kael');await until(()=>room.world.status==='playing','chapter III start');room.world.bossActive=true;room.world.hitEnemy(room.world.enemies.at(-1),99999,1);await until(()=>sb.snapshot.status==='won','chapter III won');b.send('next-chapter');await delay(100);assert.equal(room.chapter,3);assert.equal(room.round,3);b.send('restart');await startLobby([b],room);await until(()=>room.round===4,'restart chapter III alone');assert.equal(room.chapter,3);
});
