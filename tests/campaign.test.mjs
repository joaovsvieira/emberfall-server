...
test('cooperative chapters retain heroes, require boss completion, and pause for teammate menu exit',{timeout:22000},async t=>{
 ...
 room.world.bossActive=true;room.world.hitEnemy(room.world.enemies.at(-1),9999,1);await until(()=>sb.snapshot.status==='won','chapter II won');b.send('next-chapter');await delay(100);assert.equal(room.chapter,2);b.send('restart');await until(()=>room.round===3,'restart chapter II alone');assert.equal(room.chapter,2);assert.equal(room.world.players[0].hero,'kael');
});
