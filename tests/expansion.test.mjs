...
test('four real clients require all present ready, share support actions, and preserve party through chapters',{timeout:22000},async t=>{
 ...
 async function nextLobby(){await until(()=>room.stage==='lobby','next lobby');for(const c of clients)c.send('ready',true);await until(()=>[...room.members.values()].every(m=>m.ready),'ready next');a.send('start');await until(()=>room.stage==='countdown','next countdown');}
 ...
 room.world.hitEnemy(room.world.enemies.at(-1),99999,1);await until(()=>states[0].snapshot.status==='won','won');a.send('next-chapter');await nextLobby();await until(()=>states.every(s=>s.snapshot.chapter===2),'chapter II all clients');await until(()=>room.world.status==='playing','chapter II playing');room.world.bossActive=true;room.world.hitEnemy(room.world.enemies.at(-1),99999,1);await until(()=>states[0].snapshot.status==='won','won II');a.send('next-chapter');await nextLobby();await until(()=>states.every(s=>s.snapshot.chapter===3),'chapter III all clients');assert.deepEqual(room.world.players.map(p=>p.hero),['aurel','sylva','lyra','kael']);assert.ok(room.world.players.every(p=>p.hp===100&&p.shield===0));
