...
test('partial potion lot reserves every unit; shortcut consumes only remaining units and stays assigned',async t=>{
 const s=await setup(t);units(s,'healing_potion',5);await s.call('hotbar-set',{id:'alice',hero:'kael',slot:1,catalog:'healing_potion'});await s.call('market-sell',{id:'alice',item:'u0',quantity:3,price:30});const listing=(await s.call('market-list',{id:'bob'})).listings[0];assert.equal(listing.quantity,3);assert.equal((await own(s)).filter(i=>i.listed).length,3);assert.equal(itemStacks((await own(s)).filter(i=>!i.listed))[0].quantity,2);
