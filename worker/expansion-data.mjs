...
export async function expansionData(db,op,v){
 ...
 if(op==='clan-delete'){if(mine.owner!==v.id)fail('Somente o fundador pode excluir o clã.',403);await db.batch([stmt("UPDATE items SET location='inventory',bank_clan=NULL,bank_tab=NULL WHERE bank_clan=? AND EXISTS(SELECT 1 FROM clans WHERE id=? AND owner=?)",clan,clan,v.id),stmt('DELETE FROM chat_messages WHERE clan_id=? AND EXISTS(SELECT 1 FROM clans WHERE id=? AND owner=?)',clan,clan,v.id),stmt('DELETE FROM clans WHERE id=? AND owner=?',clan,v.id)]);return {ok:true};}
 fail('Ação inválida.');
}
